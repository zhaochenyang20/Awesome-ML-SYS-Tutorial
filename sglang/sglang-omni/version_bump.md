...
The MOSS failure had a direct cause: after the vocoder was merged into the pipeline process, memory accounting still treated the final process budget as though it were available during AR construction. Higgs was harder. The failure appeared on the version-bump branch, but the branch where we observed it turned out not to be the branch that introduced it.

The server started normally, both workers received requests, and every request completed. Only the timing was wrong. Higgs TTS stage-1 returned `5.046 req/s` with a mean latency of `3.114 s`, against gates of `13.64 req/s` and `1.10 s`. The same benchmark had recorded `18.189 req/s` when those gates were calibrated.

Because this was the first full run after the SGLang change, we started in the new execution path, and the profiles initially supported that decision. An Nsight Systems trace counted `34,721` CUDA stream synchronizations. `py-spy` repeatedly landed on per-token CUDA Graph buffer resets and sampling parameters copied back to the host. These were real problems. Fixing them cleaned up token handoff, `FutureMap` ownership, WAR events, and CUDA Graph execution, but recovered almost none of the missing throughput. The benchmark remained near `5 req/s`.

We then tried to isolate the dependency change by holding Omni at the commit from which the bump branch had been cut and swapping only the SGLang version beneath it. The two runs returned `4.663` and `4.636 req/s`. For a moment, that looked like proof that the upgrade was innocent and Higgs had always been this slow. The control was wrong: the throughput loss was already present in the Omni commit held constant, so changing SGLang could never expose it.

The useful question was not which part of the bump broke Higgs, but when Higgs had last been fast. Bisecting Omni between that point and the branch identified [PR #1071](https://github.com/sgl-project/sglang-omni/pull/1071), merged shortly before the version-bump work began. On the same GPU and SGLang pin, its parent commit produced `9.318 req/s`; the merged change produced `4.664 req/s`.

PR #1071 moved the Higgs vocoder into a separate process. The autoregressive engine and vocoder still occupied the same H100, but now owned different CUDA contexts. Without MPS, the GPU time-sliced between those contexts instead of overlapping their work. Replaying only that topology change in CI's dual-worker configuration moved throughput from `11.059` to `5.009 req/s`. At concurrency 16, the observed `3.11 s` mean latency accounts for the roughly `5 req/s` throughput: requests were not being dropped; context contention returned as latency.

The change had landed because the workloads were different. PR #1071 was tuned at concurrency 96 with non-default batching parameters, where isolating the vocoder was a real improvement. The CI gate runs at concurrency 16 with the defaults, where the same topology loses. GPU CI also requires a maintainer-applied label, so no post-merge run had compared those configurations before the bump branch exercised the gate.

A simple revert would have discarded a valid concurrency-96 deployment to repair the default. The final bump instead moved the vocoder back into the engine process so both stages share one CUDA context, then recovered vocoder speed inside that topology. Compiled codec decode is disabled because it loses throughput in the shared process at concurrency 16; codec decode instead uses CUDA Graphs captured for every reachable default frame count from 1 through 150. The existing gate remained unchanged, and a unit test now pins the vocoder's process assignment. The version bump did not create this regression. It supplied the first workload that made an earlier optimization fail in a different operating regime.

## What the Upgrade Required Us to Verify

These problems needed different forms of proof because they failed in different ways. The scheduler bridge was checked against the actual `0.5.16` execution path rather than inferred from renamed methods. The Qwen fix required bit-identical intermediate and final tensors, because an end-to-end accuracy score could show the regression but could not localize it. The lifecycle change required exercising abort and stream interleavings while watching memory settle after requests. The MOSS fix required reading GPU memory at the exact startup phase where the KV decision was made. Higgs required separating the Omni and SGLang histories before constructing the control, because changing the dependency beneath an already-regressed Omni commit could not identify the regression's owner.
