...
本文档基于 [sglang-omni](https://github.com/sgl-project/sglang-omni) 的 commit [2489a10](https://github.com/sgl-project/sglang-omni/commit/2489a10) 进行解读。后续在 benchmarking 与 tests 上的迭代不影响本文涉及的主框架架构。
...
Qwen3-Omni 的 Thinker-Talker 双模型架构、Talker / MTP / Code2Wav 的逐帧推理流程,以及它与 Fish Audio S2 Pro 这类 Dual-AR 模型的对比,请参见 [`transformers/omni/readme.md`](../../transformers/omni/readme.md) 中"以 Qwen3-Omni 为代表的 Thinker-Talker 模型推理"一节。本文聚焦于 SGLang Omni 在该模型架构之上的工程实现。
...
```mermaid
graph LR
    A["Request"] --> Pre["`Preprocess
CPU`"]
    Pre --> IE["`Image Enc
GPU`"] & AE["`Audio Enc
GPU`"]
    Pre & IE & AE --> Agg["`Aggregate
CPU`"]
    Agg --> Th["`Thinker
GPU:0`"]
    Th --> Dec["`Decode
...
```mermaid
graph LR
    S1["Pre"] --> S2["ImgEnc"] --> S3["AudEnc"] --> S4["Agg"] --> S5["`Thinker
(must finish all)`"] --> S6["Decode"] --> S7["`Talker
(then starts)`"] --> S8["..."]
...
```mermaid
graph LR
    CO["Coordinator"] -- "Submit" --> Pre["`preprocessing
CPU`"]
    Pre --> IE["`image_encoder
GPU`"] & AE["`audio_encoder
GPU`"]
    Pre & IE & AE --> Agg["`aggregate
CPU`"]
    Agg --> Th["`thinker
GPU:0`"]
    Th --> Dec["`decode
Terminal`"]
    Th -- "stream_to" --> Tk["`talker_ar
GPU:1`"]
    Tk -- "stream" --> CP["`code_predictor
GPU:1`"]
    CP -- "stream" --> CW["`code2wav
Terminal`"]
    CP -. "feedback" .-> Tk
    Dec & CW -- "Complete" --> CO
...
(declarative)`"] --> Compile["`compile_pipeline()
1. Allocate ZMQ endpoints
2. Create Coordinator
3. Compile each Stage
4. Wire stream targets`"]
    Compile --> Runtime["`Coordinator +
Stage₁...Stage₉`"]
    Runtime --> Start["`PipelineRunner.start()
create_task(stage.run()) × 9`"]
...
**Coordinator 不做的事**：不负责 Stage 之间的路由。"preprocessing 完了下一步去哪"是每个 Stage 自己的 `get_next` 函数决定的，Stage 之间直接通过 ZMQ 通信，不经过 Coordinator。
...
为什么分开：ZMQ 适合传小消息但传大 tensor 太慢；共享内存适合传大数据但不适合做通知。
...
```mermaid
graph LR
    Upstream["Upstream"] --> SCP["`ControlPlane
ZMQ`"] --> IH["InputHandler"] --> Router["Router"] --> W["`Workers
(Executor)`"]
    W --> Relay["`Relay
SHM/NCCL`"] --> Downstream["Downstream"]
...
**async 并发模型**：`Stage.run()`、`abort_listener()`、`worker.run()` 这三个协程跑在**同一个进程的同一个 asyncio event loop** 里，通过 `await` 协作切换——等 ZMQ 消息时不阻塞 abort 监听，等 worker 结果时不阻塞新消息接收。真正的多 Stage 并行通过 `multiprocessing` 实现——每个 Stage 一个独立进程（由 `MultiProcessPipelineRunner` 启动）。
...
graph LR
    subgraph CrossGPU["Cross-GPU: via Relay"]
        direction LR
        A1["`Stage A
GPU:0`"] --> |"GPU→CPU copy"| SHM["`Shared Memory
Relay`"] --> |"CPU→GPU copy"| B1["`Stage B
GPU:1`"]
    end
    ...
    subgraph SameGPU["Same-GPU: CUDA IPC zero-copy"]
        direction LR
        A2["`talker_ar
GPU:1`"] --> |"Pass tensor handle directly<br/>zero data copy"| B2["`code_predictor
GPU:1`"]
    end
...
为什么 Thinker 需要 `EngineExecutor` 而 image_encoder 只需要 `DirectModelExecutor`？因为 **Thinker 是自回归的**——一个请求要跑几百步 forward（每步生成一个 token），多个请求可以共享 GPU 算力（continuous batching），需要完整的 Scheduler + KV cache 管理。而 image_encoder 是**一次性的**——一张图进去，一个 embedding 出来，不需要复杂调度。
...
```mermaid
graph LR
    Input["Text + Image + Audio"] --> Media["`MediaIO
(async parallel loading)`"] --> Token["`Chat Template +
Tokenize`"] --> Out["`PipelineState
prompt, encoder_inputs,
mm_inputs`"]
...
```mermaid
graph LR
    EMB["embed_tokens"] --> Layer["`layers ×28
RMSNorm → GQA Attn → RMSNorm → MoE(128e, top-8)`"] --> NORM["norm"] --> LM["lm_head → logits"]
...
graph LR
    ...
    Th --> |"DataReadyMessage<br/>full output_ids"| Dec["`decode
(text postprocess)`"]
    Th --> |"stream_to<br/>per-token hidden states"| Tk["`talker_ar
(speech generation)`"]
...
graph LR
    Thinker["`thinker
GPU:0`"] -- "stream: hidden states" --> TA["`talker_ar
20L MoE`"] -- "stream: hidden + code" --> CP["`code_predictor
5L dense`"] -- "stream: 16L RVQ codes" --> CW["`code2wav
Vocoder`"]
    CP -. "feedback" .-> TA
...
graph LR
    ...
    TH["`thinker_hidden
(layer 24)`"] -- "multimodal positions" --> HP["hidden_projection"]
...
graph LR
    Input["`talker_hidden +
layer-0 code`"] --> AR["`AR predict
layer 1~15`"] --> Codes["16-layer RVQ codes"]
...
[OmniEngine](https://github.com/sgl-project/sglang-omni/blob/main/sglang_omni/engines/omni/engine.py) 是 Thinker 和 Talker AR 的执行引擎，核心是一个无限循环——每一步做三件事：Scheduler 选请求组 batch → ModelRunner 跑 forward → Scheduler 根据输出更新状态。Scheduler 负责"选谁、怎么组 batch"（调度策略），ModelRunner 负责"拿到 batch 跑 forward 返回"（无状态执行），两者通过 Protocol 接口解耦，Thinker 和 Talker 各有不同实现。
...
1. `_step_overlap()` 通过 `asyncio.run_in_executor()` 将 GPU forward 提交到**另一个线程**——因为 `model.forward()` 是同步函数（调用 CUDA kernel），如果在 event loop 线程里直接跑会阻塞整个事件循环。放到线程池后，GPU forward 在工作线程里跑，主线程的 event loop 继续处理其他 task（收新请求、处理 abort 等）。Python GIL 在等 CUDA kernel 返回时会释放，所以不会阻塞其他线程。
2. 在 `await future` 等待 GPU 的同时，CPU 处理上一步的 `_process_pending_result()`
...
graph LR
    Pre["`preprocessing
raw_inputs, prompt,
mm_inputs, encoder_inputs`"] --> Enc["`encoder
encoder_outs`"] --> Agg["`aggregate
thinker_inputs`"] --> Th["`thinker
thinker_out`"] --> Dec["`decode
engine_outputs`"]
...
普通 Stage 间通信是"跑完了才发"。但 Thinker 到 Talker 需要逐 token 流式传输——Thinker 每生成一个 token 就立刻把 hidden state 通过 `stream_fn` 回调发出去（写共享内存 + 发 `DataReadyMessage(chunk_id=N)`），不等生成完毕。接收端的 `StreamQueue` 是弹性缓冲区，解耦了两边的速度差异：
...
Talker 启动时如果队列里已经攒了多个 token，会一次性全部取出做 **prefill**（一次 forward 处理多个），比逐个处理更高效。之后进入 decode 阶段，后台 `_bridge_inbound` 协程持续从队列取新 token 追加到 `trailing_text_hidden`。

### Feedback 环路（Talker ↔ Code Predictor）

Talker 每生成一个 codec token → 发给 Code Predictor → 状态改为 `WAITING_FEEDBACK`（Scheduler 不再选它）→ Code Predictor 处理完返回 `summed_embeddings` → OmniEngine 检测到 feedback 到达 → `resume_request()` 恢复状态为 `WAITING` → 下一轮 `schedule()` 重新选中它。这是整个系统最复杂的状态转换。

### Abort 清理

Coordinator 广播 abort 后，每个 Stage 的 `_on_abort()` 要清理六处状态：路由器队列、输入聚合器 pending 数据、共享内存 slot、StreamQueue、通知 Executor 停止生成、唤醒 Worker 的 `await get_result()`（注入 `CancelledError`）。漏清任何一处都会导致资源泄漏。

### 多进程部署

生产环境使用 `MultiProcessPipelineRunner`（[mp_runner.py](https://github.com/sgl-project/sglang-omni/blob/main/sglang_omni/pipeline/mp_runner.py)），通过 `multiprocessing.Process` 为每个 Stage 启动独立进程。主进程只跑 Coordinator + HTTP Server，子进程各自反序列化配置、编译自己的 Stage、跑自己的 asyncio event loop。进程间通过 ZMQ + 共享内存通信，一个进程崩溃不影响其他进程。
...
SGLang Omni 是一个典型的**过度工程化**产物。30,000 行代码、7 种 ABC 接口、6 种 Protocol、4 种 Relay 后端——只为了服务一个模型。以下是基于代码逐行阅读的严厉批评。
...
**15+ 层抽象**。核心逻辑只有三件事：收请求、跑模型、发结果。其余全是胶水。
...
整个项目弥漫着"用 try/except 兜住一切"的风格。`worker/runtime.py` **673 行代码里有 18 个 try/except 块**，平均每 37 行一个。
...
这种代码的本质是：**用 "程序不崩溃" 掩盖 "程序在跑错误的东西"**。一个请求静默失败，上层拿到空结果，用户看到莫名其妙的输出，开发者去 log 里翻半天才发现三层 try/except 之前的那个异常才是根因。

### 3. God Class：TalkerStreamingExecutor

`talker_executor.py` 是整个代码库最失控的文件——**937 行，26 个方法，一个类干了至少 5 件完全不同的事**：
...
`request.data` 没有类型定义，没有 schema，字段名全靠字符串硬编码。改了一个字段名，不会有任何编译错误——只有运行时 `getattr` 返回默认值，然后模型默默产出错误结果。
...
`mooncake.py` 的 `__init__` 有 247 行——一个构造函数比大多数完整的类都长。里面混杂了设备解析、内存池分配、credit 初始化、listener task 创建，任何一个都应该是独立方法。
...
全项目 **392 处** `is not None` 检查——平均每 77 行代码就有一处在检查 None。
...
4 个 Relay 后端（shm, nccl, nixl, mooncake）共 1600+ 行。它们之间有大量重复的 credit 管理、slot 分配、cleanup 逻辑，但**每个实现都从头写一遍**。`Relay` 基类定义了接口但没有公共实现，完全是一个空壳 ABC。
