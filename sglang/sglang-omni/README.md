...
回答：对，这里的重点确实不该落在“sglang 能不能支持 encoder / vocoder / MTP”这种组件级问题上。更核心的是两个接近同量级的 decode 主干如何做 placement，以及对应的 KV cache、显存和带宽如何切分。换句话说，Qwen3-Omni 真正把问题从“单模型能力扩展”推到了“双主干资源协同 + 跨模型时序编排”。
...
回答：这个提醒也成立。更准确的说法不是“必须重新造一个很重的新框架”，而是“需要在已有 SGLang 抽象之上补一层最小够用的 runtime 编排”。encoder、单模型 decode、部分流式能力都可以直接复用；新增复杂度应尽量收敛在 Thinker / Talker 协同、feedback 环路和终态聚合上，而不是把所有东西都重新发明一遍。
...
回答：可以这么概括，但关键词不是单纯的“多进程”，而是“按 stage 划分设备边界、调度边界和生命周期边界的异步生产者-消费者模型”。多进程只是当前最自然的落地方式，不是唯一目的；真正的目的，是把不同模型的资源管理和时序协同清楚地切开。
...
回答：当前实现的是 per-request 的 best-effort abort：Coordinator 广播 `AbortMessage`，各个 Stage 在 `_on_abort()` 里清理本地聚合状态、router 绑定、relay、stream queue，并通知 executor `abort(request_id)`。它不是一个带“退出队列 / graceful drain”语义的协议，所以并不保证某个已经运行到一半的 encoder 一定把结果继续稳稳送到 Thinker。更准确地说，当前语义是“尽快停止并让后续 stage 丢弃这条请求”，而不是“即使 abort 也要把 in-flight 结果安全冲刷到底”。
...
回答：`AbortMessage` 是 request 级取消，针对某个 `request_id` 广播给所有 Stage；`ShutdownMessage` 是 stage 级生命周期控制，由 Coordinator 在整体停机时逐个 Stage 发送，让它们退出 `run()` 主循环并停止 worker。前者解决“这条请求不要了”，后者解决“整个 Stage 进程准备关机”。
...
回答：这个感受本身说明了问题：端到端时序如果一口气把内部层次全摊开，阅读负担会很重。更合理的压缩方式是只抓四个结构点：预处理扇出、aggregate 汇聚、Thinker 分叉、Talker ↔ Code Predictor feedback 环。Stage 内部那些 Router / Worker / Executor / DataPlane 的实现细节，放到后文按层拆开讲会更清楚。
...
回答：这里确实可以先把它当成理解侧的 audio tower，而不是生成侧的 RVQ codec 模块。它做的是 `Mel 特征 -> audio_embeds` 的表征编码，并返回下采样后的长度信息，供 Thinker 做多模态理解；RVQ 那套 codebook 补全和波形重建属于后面的 Code Predictor / Code2Wav 路线，不在这个 encoder 里。
...
回答：严格说这里更像 detokenize 或 text postprocess，而不是模型意义上的 decode。之所以文档仍叫 Decode Stage，是因为它在 pipeline 语义上承担的是“把 Thinker 的 `output_ids` 转成用户可消费的文本事件与最终结果”这一步：既包括 token -> text，也包括 stream delta / final event 的整理。
...
回答：这段不能直接删，因为它解释了 speech pipeline 里最关键的一步：Talker 只负责生成 layer-0 codec token 和对应 hidden state，Code Predictor 再按步补齐剩余 layer 1~15，形成完整 16-layer RVQ codes 给 Code2Wav，同时把 `summed_embeddings` 反馈回 Talker 作为下一步条件。没有这一步，就既解释不清 feedback 环，也解释不清为什么 Code2Wav 不是直接接 Talker。
