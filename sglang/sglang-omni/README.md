...
graph TB
    ...
    subgraph Thinker["Thinker · GPU:0"]
        direction TB
        T1["28 层 MoE Transformer"]
        T2["128 experts, top-8 routing"]
        T3["理解多模态输入, 生成文本 token"]
        T1 --- T2 --- T3
    end

    subgraph Talker["Talker · GPU:1"]
        direction TB
        K1["20 层 MoE Transformer"]
        K2["128 experts, top-6 + shared expert"]
        K3["将文本 token 转为语音 codec token"]
        K1 --- K2 --- K3
    end

    subgraph SpeechPipeline["语音合成流水线"]
        CP["Code Predictor<br/>RVQ 多层码预测"]
        C2W["Code2Wav<br/>Vocoder 合成波形"]
        CP --> C2W
    end

    text & img & audio --> Thinker
    Thinker -- "文本 token + hidden states<br/>(逐 token 流式传输)" --> Talker
    Thinker -- "文本 token" --> TextOut["文本输出"]
...
```mermaid
gantt
    title 边想边说：Thinker 与 Talker 流式并行
    dateFormat X
    axisFormat %s
...
```mermaid
graph LR
    subgraph Traditional["传统 TTS"]
        direction LR
        A1["文本字符串"] --> A2["TTS 模型"] --> A3["语音"]
    end

    subgraph Qwen3["Qwen3-Omni Talker"]
        direction LR
        B1["Thinker hidden states<br/>+ 文本 token"] --> B2["Talker"] --> B3["语音 codec"] --> B4["波形"]
    end
...
graph LR
    subgraph TalkerAR["Talker AR"]
        TA1["自回归采样"]
        TA2["生成 1 个<br/>layer-0 codec token"]
        TA1 --> TA2
    end

    subgraph CodePredictor["Code Predictor"]
        CP1["接收 layer-0 code<br/>+ talker_hidden"]
        CP2["自回归预测<br/>layer 1~15"]
        CP3["组成 16 层<br/>RVQ codes"]
        CP1 --> CP2 --> CP3
    end

    subgraph Code2Wav["Code2Wav"]
        CW1["Vocoder"]
        CW2["24kHz<br/>float32 音频"]
        CW1 --> CW2
    end

    TA2 -- "stream:<br/>talker_hidden +<br/>codec token" --> CP1
    CP3 -- "stream:<br/>16 层 codes" --> CW1
...
sequenceDiagram
    ...
    loop 每个时间步
        T->>T: 采样 codec token (layer-0)
        T->>C: stream: talker_hidden + codec_token
        T->>T: 进入 WAITING_FEEDBACK

        C->>C: 自回归生成 layer 1~15
        C->>W: stream: 16 层 RVQ codes
        C-->>T: feedback: summed_embeddings

        T->>T: resume, 注入 feedback
        W->>W: 累积 chunks → Vocoder 合成
    end
...
```mermaid
graph TB
    subgraph Thinker["Thinker 逐 token 生成"]
        direction LR
        T1["token₁"] --> T2["token₂"] --> T3["token₃"] --> T4["token₄"] --> T5["..."]
    end

    subgraph Projection["text_projection 投影"]
        direction LR
        P1["proj₁"] --> P2["proj₂"] --> P3["proj₃"] --> P4["proj₄"] --> P5["..."]
    end

    subgraph Queue["trailing_text_hidden 队列 (实时追加)"]
        direction LR
        Q["[proj₁, proj₂, proj₃, proj₄, ...]"]
    end

    subgraph Talker["Talker 消费 → 生成语音 codec"]
        direction LR
        S1["codec₁"] --> S2["codec₂"] --> S3["codec₃"] --> S4["codec₄"] --> S5["..."]
    end
    ...
    style Thinker fill:#4a9eff,color:#fff
    style Talker fill:#ff6b6b,color:#fff
    style Projection fill:#e9ecef,color:#333
    style Queue fill:#fff3bf,color:#333
```
...
```mermaid
graph LR
    A["用户请求"] --> B["Tokenize"] --> C["一个模型 Forward"] --> D["Sampling"] --> E["Detokenize"] --> F["返回"]
...
```mermaid
graph TB
    A["用户请求<br/>文本+图片+音频"] --> Pre["预处理<br/>CPU"]
    Pre --> IE["图片编码<br/>GPU"]
    Pre --> AE["音频编码<br/>GPU"]
    Pre --> Agg["聚合<br/>CPU"]
    IE --> Agg
...
```mermaid
graph TB
    subgraph Serial["串行单进程"]
        direction LR
        S1["Pre"] --> S2["ImgEnc"] --> S3["AudEnc"] --> S4["Agg"] --> S5["Thinker<br/>(全部完成)"] --> S6["Decode"] --> S7["Talker<br/>(才开始)"] --> S8["..."]
    end

    subgraph Async["多 Stage 异步流水线"]
        direction TB
    ...
    style Serial fill:#ffe3e3,color:#333
    style Async fill:#d3f9d8,color:#333
...
```mermaid
graph TB
    subgraph Coordinator["Coordinator (请求路由 / 完成聚合 / Abort 广播)"]
        direction LR
        CO["Coordinator"]
    end

    subgraph TextPipeline["文本处理流水线"]
        direction LR
    ...
    subgraph SpeechPipeline["语音处理流水线 · GPU:1"]
        direction TB
        Tk["talker_ar"]
        CP["code_predictor"]
    ...
    CO -. "AbortMessage<br/>(PUB/SUB 广播)" .-> Pre & IE & AE & Agg & Th & Dec & Tk & CP & CW
...
```mermaid
graph TB
    subgraph Config["PipelineConfig (声明式)"]
        C1["entry_stage / terminal_stages"]
        C2["StageConfig × 9"]
        C3["gpu_placement"]
    end

    subgraph Compile["compile_pipeline()"]
        direction TB
        S1["1. 分配 ZMQ 端点<br/>每个 Stage 一个 IPC/TCP 地址"]
        S2["2. 创建 Coordinator<br/>PULL bind + PUB bind"]
        S3["3. 逐个编译 Stage"]
        S4["4. 连接流式通道<br/>wire stream_to, StreamQueue"]
        S1 --> S2 --> S3 --> S4
    end

    subgraph StageCompile["编译单个 Stage"]
        direction TB
        SC1["import_string(executor_factory)"]
        SC2["import_string(get_next)"]
        SC3["创建 Stage 实例<br/>StageControlPlane + Relay"]
        SC4["创建 Worker(executor)<br/>绑定到 Stage"]
        SC5["coordinator.register_stage()"]
        SC1 --> SC2 --> SC3 --> SC4 --> SC5
    end

    subgraph Runtime["可运行对象"]
        R1["Coordinator"]
        R2["Stage₁ ... Stage₉"]
    end

    subgraph Start["PipelineRunner.start()"]
        direction TB
        ST1["coordinator.start()"]
        ST2["create_task(run_completion_loop)"]
        ST3["create_task(stage.run()) × 9"]
        ST1 --> ST2 --> ST3
    end
    ...
    style StageCompile fill:#fff3bf,color:#333
...
graph LR
    ...
    CO --> |SubmitMessage<br/>PUSH| Entry["入口 Stage"]
    ...
    CO --> |合并 partial_results| Client

    CO -. "AbortMessage<br/>PUB 广播" .-> Stages
...
`[ControlPlane](https://github.com/sgl-project/sglang/blob/main/python/sglang_omni/pipeline/control_plane.py)` 基于 ZMQ 实现进程间通信，使用两种消息模式：
...
```mermaid
graph TB
    subgraph PUSHPULL["PUSH/PULL 模式 (点对点)"]
        direction LR
...
```mermaid
graph TB
    subgraph Stage["Stage 内部结构"]
        direction TB
        CP["StageControlPlane<br/>ZMQ PULL/PUSH/SUB"]
        IH["InputHandler<br/>DirectInput 或 AggregatedInput"]
        Router["WorkerRouter<br/>请求分发"]
        Relay["Relay<br/>共享内存 / NCCL / NIXL"]
        SQ["StreamQueue<br/>流式数据队列"]
    ...
    Upstream["上游 Stage"] --> |"DataReadyMessage"| CP
    Relay --> |"DataReadyMessage"| Downstream["下游 Stage"]
...
sequenceDiagram
    ...
    alt next_stages 不为空 (非 Terminal)
...
```mermaid
graph LR
    subgraph CrossGPU["跨 GPU: 经 Relay 中转"]
        direction LR
        A1["Stage A<br/>GPU:0"] --> |"GPU→CPU 拷贝"| SHM["共享内存<br/>Relay"] --> |"CPU→GPU 拷贝"| B1["Stage B<br/>GPU:1"]
    end

    subgraph SameGPU["同 GPU: CUDA IPC 零拷贝"]
        direction LR
        A2["talker_ar<br/>GPU:1"] --> |"直接传 tensor handle<br/>零数据拷贝"| B2["code_predictor<br/>GPU:1"]
    end
...
```mermaid
graph TB
    Executor["Executor (抽象接口)"]

    Executor --> PE["PreprocessingExecutor<br/>CPU 预处理<br/>tokenize, 媒体加载"]
    Executor --> DME["DirectModelExecutor<br/>直接运行 torch 模型<br/>batch / streaming"]
    Executor --> EE["EngineExecutor<br/>包装 OmniEngine<br/>Thinker, Talker AR"]
    Executor --> FE["FusedExecutor<br/>串联多个 Executor<br/>减少 IPC 开销"]
...
sequenceDiagram
    ...
    IE->>IE: ViT 编码
    AE->>AE: Audio Transformer 编码

    par fan-in: 汇聚到 aggregate
        IE->>Agg: DataReady (image_embeds)
        AE->>Agg: DataReady (audio_embeds)
    end

    Note over Agg: 等待 3 路输入全部到达
    Agg->>Agg: merge_for_thinker()
    Agg->>Th: DataReady (thinker_inputs)

    Th->>Th: MoE Transformer 自回归生成
    par fan-out: 文本 + 语音
        Th->>Dec: DataReady (output_ids)
        Th-->>Tk: stream_to (hidden states, 逐 token)
    end

    Dec->>Dec: detokenize
    Dec->>CO: CompleteMessage (文本结果)

    loop 每个 token
        Tk->>Tk: 采样 codec token
        Tk-->>CP: stream (talker_hidden + codec)
        CP->>CP: 生成 layer 1~15
        CP-->>CW: stream (16 层 codes)
        CP-->>Tk: feedback (summed_embeddings)
    end

    CW->>CW: Vocoder 合成波形
    CW->>CO: CompleteMessage (音频结果)

    Note over CO: 等待 decode + code2wav 都完成
    CO->>C: 返回 {text + audio}
```
...
graph LR
    ...
    subgraph MediaIO["异步并行加载"]
        direction TB
        IM["ImageMediaIO<br/>PIL/URL/Base64/文件<br/>→ RGB PIL.Image"]
        AM["AudioMediaIO<br/>WAV/WebM/MP3/OGG/FLAC<br/>→ 16kHz float32"]
        VM["VideoMediaIO<br/>TorchCodec/TorchVision<br/>→ 帧序列"]
    end

    subgraph Process["处理"]
        direction TB
        CK["缓存键计算"]
        TK["Chat Template + Tokenize<br/>插入占位符 token"]
    end

    subgraph Output["PipelineState"]
        O1["prompt: input_ids + attention_mask"]
        O2["encoder_inputs:<br/>pixel_values, input_features"]
        O3["mm_inputs:<br/>原始媒体数据"]
    end
...
graph LR
    ...
    Agg["aggregate<br/>AggregatedInputHandler<br/>(等待 3 路全部到达)"]
...
```mermaid
graph TB
    subgraph ThinkerModel["Qwen3OmniMoeThinkerTextModel"]
        EMB["embed_tokens<br/>vocab → hidden=2048"]

        subgraph Layer["layers[0..27] (×28)"]
            direction TB
            LN1["input_layernorm (RMSNorm)"]
            ATTN["self_attn<br/>GQA: 28 heads, 4 kv heads<br/>+ RoPE (YARN scaling)<br/>+ RadixAttention (KV cache)"]
            LN2["post_attention_layernorm (RMSNorm)"]

            subgraph MoE["MoE Block"]
                Gate["gate: Router"]
                Experts["FusedMoE<br/>128 experts, top-8<br/>intermediate=768"]
                Gate --> Experts
            end
    ...
    style ATTN fill:#4a9eff,color:#fff
    style MoE fill:#ffa94d,color:#fff
...
graph LR
    ...
    Th --> |"DataReadyMessage<br/>完整 output_ids"| Dec["decode<br/>(文本后处理)"]
    Th --> |"stream_to<br/>逐 token hidden states"| Tk["talker_ar<br/>(语音生成)"]
...
```mermaid
graph TB
    subgraph GPU1["GPU:1 语音流水线"]
        direction TB

        subgraph TalkerAR["Stage 7: Talker AR"]
            TA["20 层 MoE Transformer<br/>128 experts, top-6<br/>+ Shared Expert"]
        end

        subgraph CodePred["Stage 8: Code Predictor"]
            CP["5 层 dense Transformer<br/>hidden=1024, vocab=2048"]
        end
        ...
        CP -- "stream:<br/>16 层 RVQ codes" --> CW
        CP -. "feedback:<br/>summed_embeddings" .-> TA
    end

    Thinker["thinker (GPU:0)"] -- "stream_to:<br/>hidden states<br/>逐 token" --> TA

    CW --> Audio["音频波形<br/>CompleteMessage"]
...
```mermaid
graph LR
    subgraph ThinkerOut["Thinker 输出"]
        TE["thinker_embed<br/>(token embedding)"]
        TH["thinker_hidden<br/>(layer 24 states)"]
    end

    subgraph Segment["按 chat template 分段"]
        User["User 段"]
        Asst["Assistant 段"]
    end

    subgraph Projection["投影"]
        TP["text_projection<br/>(文本位置)"]
        HP["hidden_projection<br/>(多模态位置)"]
    end
...
graph TB
    ...
    Input --> Embed["嵌入 layer-0 code"]
    Embed --> AR["自回归生成 layer 1~15"]
    AR --> Codes["完整 16 层 RVQ codes"]
    AR --> Sum["求和所有层 codec embedding<br/>→ summed_embeddings"]
...
stateDiagram-v2
    ...
    RUNNING --> WAITING_FEEDBACK: Talker 等待 feedback
...
graph TB
    OE["OmniEngine"]
    OE --> BP["BatchPlanner<br/>选择请求 + 构建 batch"]
    OE --> RM["ResourceManager<br/>内存资源管理"]
    OE --> IC["IterationController<br/>判断请求是否完成"]
    OE --> IP["InputPreparer<br/>SchedulerOutput → 模型输入"]
    OE --> OP["OutputProcessor<br/>模型输出 → RequestOutput"]
    OE --> CM["CacheManager<br/>可选的输出缓存"]
...
graph LR
    ...
    subgraph Enc["encoder 写入"]
        B1["encoder_outs:<br/>image_embeds<br/>audio_embeds"]
    end

    subgraph Agg["aggregate 写入"]
        C1["thinker_inputs:<br/>合并后的完整输入"]
    end

    subgraph Th["thinker 写入"]
        D1["thinker_out:<br/>output_ids, hidden_states"]
    end

    subgraph Dec["decode 写入"]
        E1["engine_outputs:<br/>最终文本"]
    end
