...
```mermaid
graph TB
    subgraph Serial["Sequential (single process)"]
        direction LR
        S1["Pre"] --> S2["ImgEnc"] --> S3["AudEnc"] --> S4["Agg"] --> S5["Thinker<br/>(must finish all)"] --> S6["Decode"] --> S7["Talker<br/>(then starts)"] --> S8["..."]
    end

    subgraph Async["Multi-Stage async pipeline"]
        direction TB

        subgraph Row1["Parallel encoding"]
            direction LR
            A1["ImgEnc GPU"]
            A2["AudEnc GPU"]
        end

        subgraph Row2["Streaming"]
            direction LR
            B1["Thinker GPU:0"]
            B2["Talker GPU:1"]
            B1 -- "per-token stream" --> B2
        end
