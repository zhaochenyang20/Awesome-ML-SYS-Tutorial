...
沿用前面 8 条请求分成 A 到 E 五组的例子，wait-all 会让这 8 条请求的并发名额一直占用到整批完成，推迟它们对应的补发。不过，系统中另外的在途请求仍可能执行上游计算或完成并触发补发，因此不能由此断言 GPU 被 Code2Wav 独占。

Early-emit 则会在 Group A 完成后先返回它的 3 条结果，允许客户端更早补入对应的新请求。新请求经过网络与预处理后，其 Thinker / Talker 工作可能与剩余的 Code2Wav 计算发生资源竞争，从而拉长后续组的完成时间。下面展示的是这一反馈关系的假设性示意，**不是实测 GPU 时间线，也不表示各阶段的实际时长**：

```mermaid
flowchart TD
    A[Group A 完成：3 条结果已就绪] --> W[Wait-all：暂不返回这 3 条结果]
    W --> E[等待 Group B 到 E 完成]
    E --> R[整批返回 8 条结果]
    R --> C[释放对应名额并补发请求]
    A --> F[Early-emit：先返回 Group A 的结果]
    F --> N[提前释放 3 个名额并补发请求]
    N --> U[新请求经过网络、预处理与上游计算]
    U -. 可能增加共享 GPU 上的资源竞争 .-> V[影响剩余 Group B 到 E 的完成时间]
```
