...
手动去管理上万个进程的 CPU 核区域是不现实的原因。第一，`taskset` 等等方法只能约束当前进程，同机上其他进程照样能跑进当前进程的核区，隔离是单方面的；第二，服务器拓扑并不只是核编号那么简单，同一个物理核上通常有两个超线程，一旦把一对 sibling 分给两个进程，就会制造争用；第三，serving 进程本身的拓扑也是非常灵活多样的，[DP + MPS 方法](https://sgl-project.github.io/sglang-omni/basic_usage/mps_dp.html)、以及后续的 [Stage Level DP](https://github.com/sgl-project/sglang-omni/issues/1307)，都要求按 stage、按副本细粒度分核，核的数量和归属会随配置变，很难完全手动配置。人工方案覆盖不了完整的真实 serving 拓扑，也控制不住同机的其它任务。
...
只约束本进程，收益只有 10% 到 15%；对同机负载一并约束，吞吐是 2.5 倍。由此可见，想要得到隔离的收益，必须有一份覆盖全机的 plan，由 serving flag 统一约束。为了补上这个缺口，我们做了一个能感知拓扑的 CPU allocator（[PR #1463](https://github.com/sgl-project/sglang-omni/pull/1463)）。

具体而言，各模型的 host 开销差别很大，profile 显示 host 时间的大头是 stage 间调度和请求构建（Fun-ASR 约 45 ms，Qwen3-ASR 约 35 ms，音频预处理只占个位数百分比），host 处理时间占比越高的模型对争用越敏感。Fun-ASR 饱和时，ASR 的 host 处理大约使用 4.2 核（pre-LM encoder + scheduler），在配置里向上取整，最终声明 5 个独占物理核。allocator 负责读取声明，从 sysfs 里完成物理核和超线程的配对，把这 5 个物理核连同核上的两个超线程一同隔离，让 ASR 独占，serve 进程树里其余进程进入共享池。
