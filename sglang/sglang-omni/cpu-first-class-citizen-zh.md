...
CI 的问题到这里算是告一段落了，与此同时，既然 CI 里对 CPU 的隔离行之有效，我们期待生产端也应该有不错的潜在收益。接下来这部分将会讲述我们把 CPU 资源隔离的想法推到生产端并进行部署得到的结论。事实上，CPU 资源在生产端的隔离并没有显着提高，把这套隔离搬进生产部署检测效果，平均提升只有 +0.43%，各档变化在 -3.68% 到 +5.34% 之间，没有稳定的正向收益。即便如此，我们仍旧分享这一有趣的探索过程。

### 优化复用

最初，我们认为 CI 里避免掉的 CPU 争用，生产环境理论上也会出现，所以我们想把同一套隔离迁过去。动手之前，我们先看生产环境的实际情况。对一台共享 GPU 主机做简单分析，几万个 Python 任务里，97% 没有绑任何核，可以跑在全机任意核上。手动管理 CPU 核心在 serving 侧的实际采用率接近零。即便用 `taskset`，也覆盖不了服务器的完整拓扑：它只能约束本进程，管不住同机的其它任务；还容易把同一个物理核的两个超线程分给两个进程，制造争用。DP + MPS，以及后续的 Stage Level DP，都需要按 stage 和副本细粒度分核，手动划分做不到。

【TODO：你这里不是说要观测生产环境吗？你应该比较的是生产环境和我们这种开发环境的区别吧？怎么你这里只提了共享 GPU 主机呢？那这不就是开发环境吗？】
...
【TODO：这个地方的写作也让人很费解。你前面都在讲生产环境了，这一大块其实又是在分析它在我们的开发这种混用机器下面的作用。我觉得你上面这些分析应该把它挪到前一个部分，讲它在 CPU 被征用情况下的效果，然后生产形态下面的事情根本就不用分析这种混合状态。】
