...
CI 的问题到这里算是告一段落了，与此同时我们想把这个方法论也搬运到生产环境中，我们想既然 CI 里隔离确实有效，搬到生产应该也可以取得很不错的收益。但是结果恰恰相反。把这套隔离搬进生产形态一测，平均只有 +0.43%，各档变化在 -3.68% 到 +5.34% 之间，没有稳定的正向收益。这个结果让我们非常意外，这也印证了开头那句话：任何反常的现象，要么是我们的认知不够，要么是出现了奇怪的 bug。这次优化的尝试也让我们对 CI 和线上环境的不同，以及分别的 CPU 治理对策有了更加深刻的理解。

## 优化复用的研究

先讲我们为什么会预期大收益。CI 里避免掉的争用，生产环境理论上也会出现，所以我们想把同一套隔离迁过去。动手之前，先看生产环境实际长什么样。对一台共享 GPU 主机做简单分析，几万个 Python 任务里，97% 没有绑任何核，可以跑在全机任意核上。手动管理 CPU 核心在 serving 侧的实际采用率接近零。即便用 `taskset`，也覆盖不了服务器的完整拓扑：它只能约束本进程，管不住同机的其它任务；还容易把同一个物理核的两个超线程分给两个进程，制造争用。DP + MPS，以及后续的 Stage Level DP，都需要按 stage 和副本细粒度分核，手动划分做不到。
...
这一块数据，也是最让我们感到意外的部分。我们保持生产里更常见的形态重新测了一遍：同卡 DP + MPS，机器上没有外部 CPU 负载，唯一变量是 allocator 关还是开 static。
...
成也萧何，败也萧何。这个设想和这些实验本身都是有效的：在有争用的机器上，统一管控就是 2.5 倍。问题出在环境假设上。CI 的机器是共享的：每小时几十个 job，动辄一次整机重编译，争用是常态。生产恰恰相反：一台机器上就那几个服务，每个模型的 host 编排满打满算只用两到五个核，即使我们大幅度使用 DP 等技术进行并行部署，也只会用掉 20-30 个核心，而一台服务器 CPU 有上百核因此 CPU 不是稀缺资源。为高争用环境设计的保护，搬到没有争用的环境里，自然测不出收益。
