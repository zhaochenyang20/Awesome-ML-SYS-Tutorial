...
最终的结论是，ASR CI 出现严重下滑的根本原因在于主机 CPU 资源的波动：显存有 [`mem_fraction_static`](../kvcache-code-walk-through/mem-fraction-static.md) 这样的预算机制，GPU 算力会被 CI 的 scheduling 自动分配，但是 CPU 却没有任何管理机制。同时我们发现 flashinfer JIT 编译造成了非常大的 CPU 负担。旧流程下 CI 冷启动会触发一次整机重编译，我们在 [PR 1343](https://github.com/sgl-project/sglang-omni/pull/1343) 验证期间实测过，一次这样的编译，能把同机的 ASR 服务从 122 qps 打到 41。需要说明的是，flashinfer 的 kernel 走的是 JIT 编译：用到某个 kernel 时，如果缓存里已经有编译好的产物，就直接加载，毫秒级完成；只有找不到产物，才会现场编译几十分钟。所以整机重编译的本质是缓存不命中，而当时的缓存有三个不命中的原因。其一，CI 镜像虽然预热了一批编译产物，但没有覆盖 cutlass MOE 这一族 kernel。其二，镜像预热用的是 Python 3.12，而 CI 会在 job 里另建一个 Python 3.11 的 venv，版本不一致会让已有的预热产物全部被判定失效。其三，现场编译出来的结果写在 job 容器内部，容器一销毁就丢，下一个 PR 只能从头再编。最终的修复就是让缓存必定命中：新镜像把包括 cutlass MOE 在内的编译产物预热齐全，[PR 1343](https://github.com/sgl-project/sglang-omni/pull/1343) 再把 CI 环境对齐到镜像的 Python 3.12，直接复用镜像内的 Torch 与 FlashInfer。产物齐了、版本对了，每个 kernel 都能直接从镜像加载，现场编译就不再发生。修复之后，fused_moe 的首次调用从 481 秒的编译变成 1.4 秒的加载。
