...
最终的结论是，ASR CI 出现严重下滑的根本原因在于主机 CPU 资源的波动：显存有 [`mem_fraction_static`](../kvcache-code-walk-through/mem-fraction-static.md) 这样的预算机制，GPU 算力会被 CI 的 scheduling 自动分配，但是 CPU 却没有任何管理机制。同时我们发现 flashinfer JIT 编译造成了非常大的 CPU 负担。旧流程下 CI 冷启动会触发一次整机重编译，我们在 [PR 1343](https://github.com/sgl-project/sglang-omni/pull/1343) 验证期间实测过，一次这样的编译，能把同机的 ASR 服务从 122 qps 打到 41。需要说明的是，当时的 CI 镜像虽然预热了一批 flashinfer 编译产物，但没有覆盖 cutlass MOE 这一族 kernel，而 `FLASHINFER_WORKSPACE_BASE` 指向 job 容器内部，编译结果随容器一起销毁，于是每个 per-PR 的 venv 都要重新编译 30 到 60 分钟。最终的修复是把编译产物完整打包进 CI 镜像，并让 CI 真正用上它们：新镜像把包括 cutlass MOE 在内的产物预热齐全，[PR 1343](https://github.com/sgl-project/sglang-omni/pull/1343) 再把 CI 环境对齐到镜像的 Python 3.12（此前 CI 在 job 里另建 Python 3.11 的 venv，版本不一致会让预热产物整体失效），直接复用镜像内的 Torch 与 FlashInfer，fused_moe 的编译时间从 481 秒降到 1.4 秒。我们也提出过把 JIT workspace 持久化到 per-PR CI home 的备选方案（[PR 1297](https://github.com/sgl-project/sglang-omni/pull/1297)），在镜像方案生效后未再推进。
