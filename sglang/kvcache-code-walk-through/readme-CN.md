...
- **访问：**
  - 键：Token ID（沿树路径从 root 到当前节点拼接出完整前缀）
  - 值：Token 的 KV cache索引
  - 说明：能够跨 Request 复用的是**完整前缀**对应的 KV Cache，而非孤立的单个 token。由于 transformer 中第 N 层(N>0)的 K/V 是基于前面所有 token 的 hidden state 计算得到的，单个 token 的 KV 强依赖于其上文；只有当从位置 0 开始的整段前缀完全相同时，对应 KV Cache 才与具体 Request 无关、可被复用。这也是 `tree_cache` 必须使用树结构(而非简单的 token 级哈希表)的根本原因。
