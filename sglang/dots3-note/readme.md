...
Verify 环节能够成立的前提，是模型真的具备评价自己的能力，而 dots 的做法并不是靠 prompt 硬凑，是在 RL 阶段就把"评价当前状态"当作与"推进任务"并列的优化目标来训，方法叫 [TEMPO](https://studio.dots.ai/dots/tempo-blog.html)。

超长程 RL 的困难可以拆成两条，而这两条并不是同一个问题。第一条是反馈周期过长，终局奖励要等轨迹结束才能观测到，当一次 rollout 持续数十小时，任务本身的物理执行时间就构成了信号获取周期的下限，这一条加卡也无解；第二条是信用分配困难，horizon 越长，早期决策与终局奖励之间的行为链就越长，越难判断哪些中间状态真正影响了结果，这一条与有多少时间无关，是统计问题。

两条困难指向同一个解法，也就是在轨迹结束前估计当前状态的未来回报，这正是 actor-critic 一直在做的事情。但传统的 scalar value head 通过一次固定计算量的前向给出估值，无法随状态复杂度追加算力，而作者的论证是：当 actor 本身依赖 test-time scaling 才能解题时，估计它的中间状态同样是一个复杂推理问题，critic 需要回顾交互历史、检查已经形成的假设、判断当前搜索方向是否可行、推演后续路径。

> 既然 actor 的能力可以随 test-time compute 增长，用于评价这些状态的 critic 也应具备同样的能力。

### macro-step

顺着上面那句话往下，把 critic 换成生成模型的代价立刻就到来了：单次估值要生成一段较长的推理，可能还包含反思与工具调用，成本远高于 scalar head 的一次前向，因此它不可能像传统 critic 那样在每个 token 或每个动作之后密集运行。

TEMPO 的处理方式是把估值放在固定边界上，将连续 k 轮的模型—环境交互定义为一个 macro-step，估值只在终点做一次。作者给的理由是，在 Agent 任务中，有实际意义的状态变化通常发生在推理与工具调用完成、环境返回新观测之后，因此把多轮交互聚合起来，既给 actor 留出了完成一段完整探索的空间，也让 critic 能够在信息更充分的状态上估值。

而 macro-step 除了是估值的边界之外，同时也是 rollout 与优化的基本单位：每轮训练从一批已保存的中间状态及其交互历史开始，环境也恢复到对应状态，actor 从这里继续执行 k 轮，产出本轮的训练数据与新的终点状态，终点状态及其历史随后被保存下来，供后续训练轮次继续 rollout。如此以来，同一个任务可以跨多个训练轮次持续向后推进，而不必每次都重跑一遍轨迹前缀。
...
这张表里前两列的差别在算法，第三列的差别则在工程，而对 Infra 真正有影响的恰恰是第三列，它把一条 40 小时的轨迹拆成了若干个可以独立调度的段，每段都能立刻产出训练数据。这与 partial rollout 是同一条脉络上的东西，区别只在于 partial rollout 缓存的是未完成请求的生成状态，而 TEMPO 保存的是 actor 与环境的联合状态。
