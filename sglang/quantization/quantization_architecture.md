...
2. **实现三个核心方法**：
   ...
   - `process_weights_after_loading`（可选）：在权重加载完成后执行再量化、打包、转置等操作。
...
### 4.4 量化推理
- 线性层执行 `Fp8LinearMethod.apply`：量化输入、调用选定内核进行 FP8 GEMM、反量化输出：
  ```python
