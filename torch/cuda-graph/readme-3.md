...
需要强调的是，虽然两个 AR 过程共享同一个 CUDA Graph，它们各自的 KV Cache 仍然是完全独立管理的——slow head 使用 SGLang 的 paged KV cache，fast head 使用 static 预分配的 KV cache。统一 graph 只意味着两者的 kernel 被录进同一个 DAG，并不意味着它们的状态管理有任何耦合。
...
值得一提的是，PR #153 的"统一单 graph"并非唯一可行的方案。Fish Audio 团队内部曾采用另一种架构：**为 Slow AR 和 Fast AR 各 capture 一个独立的 CUDA Graph**（在 PR #153 的讨论中，sdli1995 提到了 `capture_fastlayer_one_batch_size` 和独立的 `fast_graphs`，日志中 `LLM Capture` 和 `Fastlayer sample capture` 是分开进行的）。

两种方案的对比：

| 维度 | PR #153：统一单 Graph | Fish Audio 内部：双 Graph |
|---|---|---|
...
PR #153 选择统一方案的核心收益是**消除两个 graph 之间的 CPU 调度开销**——对于 TTS 这种 latency-sensitive 的场景，每个 decode step 少一次 CPU-GPU 往返是有意义的。但需要注意，无论哪种方案，两个 AR 的 KV Cache 都是独立管理的——统一 graph 改变的是 kernel 的组织方式，不改变状态管理。
