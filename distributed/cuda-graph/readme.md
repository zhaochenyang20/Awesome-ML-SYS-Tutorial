...
CUDA Graph 在捕获 (capture) 阶段所需的显存量，主要由构成计算图的各个操作及其涉及的数据量共同决定。捕获过程实际上是在记录一系列 GPU 操作（如核函数启动、内存拷贝、内存设置等），以及这些操作所引用的所有内存区域。因此，捕获时的显存占用直接反映了模型在该特定执行路径下所需的最大瞬时显存峰值。


1. 模型参数大小：模型的所有权重（如线性层、卷积层、注意力机制的权重）和偏置，在捕获图时都必须加载到 GPU 显存中。
2. 输入和输出数据大小：用于捕获图的 dummy input 的批次大小和序列长度直接决定了捕获时激活值和中间变量所需的显存。捕获图时，系统会执行一次完整的计算图轨迹（例如，推理中的前向传播，或训练中的前向和反向传播），以确定所有内存操作和其峰值需求。因此，输入数据本身的显存占用以及它在计算过程中引发的输出和中间结果的显存占用，都会被计入。
3. 中间激活值大小：在捕获图时，CUDA 会跟踪这些激活值在内存中的分配，确保图在重放时能正确地引用它们。激活值的峰值通常发生在模型中宽度最大的层或特定算子（如多头注意力）的中间计算阶段。对于训练，反向传播所需的中间激活值（或重计算所需的激活值）也会被计入。
4. 算子临时工作空间：许多高性能的 GPU 算子库（如 cuBLAS、cuDNN、cuFFT 等）在执行复杂的计算（如大型矩阵乘法、卷积）时，可能需要额外的临时工作空间 (workspace memory) 来存储中间结果或进行内部优化。这部分内存通常由驱动程序或库函数动态请求，并被捕获为图中的内存操作。
5. 图本身的管理开销： 虽然 CUDA Graph 作为一个蓝图本身很小，但其在 GPU 驱动层面的实例化 (instantiation) 和管理也需要少量显存。这包括存储图的拓扑结构、节点信息、以及相关的内部元数据。但这部分开销通常远小于数据和激活值带来的开销。

**总的来说，CUDA Graph 捕获时的显存占用，实质上是该计算图在执行过程中所需的最大瞬时显存量。**CUDA Graph 在捕获阶段会记录所有显存操作，包括 `cudaMalloc`、`cudaMemcpy`、`cudaMemset` 等。当图被实例化并执行时，所有这些被记录的内存分配都必须已完成且地址稳定。这意味着，图所需要的最大显存量是在捕获时就被确定并需要预先分配好的。如果模型在推理时输入的形状是动态变化的（例如，不同的 sequence length），那么为了捕获一个可以处理所有可能形状的图，通常会为最坏情况（即最大输入长度）预留显存，这在某些情况下可能导致显存的浪费。

此外，为了避免频繁的显存分配和释放开销，同时保持 CUDA Graph 的有效性，通常会使用 CUDA 内存池 (CUDA Memory Pool)。内存池会预先向驱动程序申请一大块显存，然后在其中进行细粒度的分配和回收，而不会将显存真正归还给操作系统。这意味着在图执行期间，只要总的内存需求不超过内存池的大小，就可以高效地复用显存，而不会出现虚拟地址变化导致图失效的问题。
