...
## 以 Qwen3-Omni 的 Thinker-Talker 模型推理
...
2. Thinker：30B-A3B MoE Transformer，接收所有模态输入，自回归生成 text token 作为响应，activated parameters 3B。

2. Talker：3B-A0.3B MoE Transformer，语音生成的主干。自回归生成第 0 个离散 codec token，有自己独立的 KV cache，activated parameters 0.3B。

3. MTP Module（Multi-Token Prediction）：80M 参数的轻量 dense Transformer。在 Talker 每步生成第 0 个 codec token 后，MTP 负责补全剩余的固定数目的 codec token。功能上等价于 S2 Pro 的 Fast AR，但参数更小。

4. Code2Wav：200M 参数的 causal ConvNet，替代了 Qwen2.5-Omni 中的 block-wise DiT。接收一帧完整的多个离散 codec token，解码成音频波形。角色等价于 S2 Pro 的 Codec Decoder。
...
Talker prefill 完成后，开始运行自己的 decode loop。Talker 没有自己的 text embedding 层（一般的实现显式删除了继承的 `embed_tokens`，替换为 `codec_embedding` 专门用于 codec token）。Thinker 的信息通过两个独立的 `ResizeMLP`（Linear → SiLU → Linear）进入 Talker：

- `text_projection`：接收 Thinker 的 layer 0 word embedding（即 Thinker 输入层的 token embedding，不是深层 hidden states），从 Thinker hidden dim 投影到 Talker hidden dim
- `hidden_projection`：接收 Thinker 指定中间层的 hidden states，只取 audio/visual 位置（如前文所述按模态身份选取），投影到 Talker hidden dim
