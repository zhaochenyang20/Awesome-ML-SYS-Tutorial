...
S2 Pro 的 codec 使用 10 层 RVQ，也就是说每个时间步会产出 10 个 codec token id。前文说过，RVQ 的 N 个 token id 可以被展平、拆成多条流、或者由不同子网络分阶段预测。如果选择最朴素的展平方案——让一个 LLM 沿时间轴直接逐帧生成全部 10 层——序列长度膨胀 10 倍，一段 10 秒的音频（约 210 帧）变成约 2100 个 token 位置，context window 压力陡增。

Dual AR 选择的是"由不同子网络分阶段预测"这条路，而且分法正是利用了 RVQ 的层间信息不对称性：**Layer 0 承载语义，后 9 层承载声学细节；那就用一个大模型沿时间轴只管 Layer 0，再用一个小模型沿 codebook 深度补全 Layer 1-9。** 大模型负责"说什么"，小模型负责"怎么说"——序列长度回到可控范围。
