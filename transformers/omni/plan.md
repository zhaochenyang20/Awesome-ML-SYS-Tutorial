...
1. **从重构工作中的困惑切入**——不是"今天来学习 omni 模型"，而是"要重构框架，就得先吃透模型"
2. **开篇亮出 issue #188 的核心矛盾**：8-10 层抽象 vs 目标 6 层
3. **路线图 4 条以内**，直接指向驱动问题
4. **致谢随意自然**
5. **不写"本文基于 commit xxx"**——commit hash 在代码引用时自然出现

### 驱动问题

> **当我们要在一个推理框架中同时服务 S2 Pro（Dual AR）和 Qwen3-Omni（Thinker + DiT Talker）这两类架构迥异的 omni 模型时，哪些计算模式可以统一抽象为框架层的通用能力，哪些必须作为模型特异的实现留给各自的 ModelRunner？**
...
- [CUDA Graph 第三篇](../../torch/cuda-graph/readme-3.md)：CudaGraphRunner 的框架级管理，本文讨论 ModelRunner 抽象时会引用其中的设计决策
...
- 对 Diffusion Model 的初步认知：denoising 迭代过程、与 AR 的本质区别（本文会在概念章节中建立足够的直觉，不需要深入了解）
...
- **数据量差异**：语音的原始采样率（16kHz-48kHz）意味着 1 秒音频 = 16000-48000 个采样点，而 LLM 生成 1 秒对应的文本大约只需要几个 token。如果直接让 LLM 处理原始音频波形，序列长度会膨胀到不可接受的程度
- **从数据量推导出压缩需求**：需要一种中间表示，把高频的音频信号压缩成低频的离散 token，使其能被 Transformer 在可接受的序列长度下处理
- **Codec 的定义**：**Audio Codec 是一种将连续音频波形编码为离散 token 序列的神经网络，同时保留足够的信息（音色、韵律、情感）以支撑高质量的声音还原。** 可以类比为"把 WAV 压缩为 MP3，但不仅保留了音乐内容，还保留了 MP3 通常会丢弃的说话人特征"
- **数值例子**：原始 24kHz 采样 → Codec 编码后约 12.5 token/s（以 EnCodec 为例）→ 压缩比约 1920:1。一句 5 秒的话从 120000 个采样点变成约 63 个 codec token——这个量级才能让 Transformer 吃得下

#### 1.2 RVQ：Codec 的多层信息组织

从 1.1 的"单层 codec 可能不够"推导：

- **单 codebook 的信息瓶颈**：一个 codebook 能表达的信息有限。如果只用一个 codebook，要么码本太大（训练困难），要么量化误差太高（还原质量差）
- **RVQ（Residual Vector Quantization）**：用多层 codebook 逐步逼近。第一层编码粗粒度的语义信息（说了什么），后续各层编码残差（怎么说的——音色、韵律、情感细节）。这种层级结构暗示了一个重要推论：**不同 codebook 层的信息价值是不对称的——第一层携带最核心的语义，越往后越偏向声学细节**
- **对后续模型分析的意义**：S2 Pro 的 Slow AR 只生成第 1 个 codebook（语义），Fast AR 生成剩余 9 个（声学）——这种分工正是 RVQ 层级结构的直接体现
