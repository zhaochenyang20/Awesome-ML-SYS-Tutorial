...
- [CPU 启示录：重新审视 CPU 资源作为语音模型 Serving 过程的一等公民](./sglang/sglang-omni/cpu-first-class-citizen-zh.md): from a CI calibration incident to a topology-aware CPU allocator — why speech serving is host-bound, how CPU contention (not GPU) collapses throughput, and why isolation that works in CI does not automatically transfer to production. Thanks to [Jiaxin Deng](https://github.com/JiaxinD).
