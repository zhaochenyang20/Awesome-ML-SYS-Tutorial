...
- [Let Speed Be With Stability: All-In-One Solution to Training-Inference Mismatch with Miles](./rlhf/mismatch/blog-cn.md)：介绍 slime 框架对训推不一致问题提供的两种解决方案：通过 kernel 层面对齐实现完美的**True On-Policy**训练，以及基于 TIS/MIS 等算法来缓解训推不一致的影响。同样刊载[英文版本](./rlhf/mismatch/blog-en.md)。
